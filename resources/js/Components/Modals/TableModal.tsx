...
interface TableModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSave: (tableName: string, fields: TableField[], fileKeyName: string, fileNameRenamed: string, fileNameShort: string) => void;
...
export default function TableModal({ mode, isOpen, onClose, onSave, table, loading = false, schemaVersionId }: TableModalProps) {
  ...
  // Initialize form when modal opens or table changes
  useEffect(() => {
    if (!isOpen) return;
    ...
    if (isEditMode && table) {
      ...
      // Convert SchemaField[] to TableField[]
      ...
      // Create mode: start with default field
      resetForm();
      if (schemaVersionId) {
  ...
  // Auto-select fileKeyName when fields change and none selected
  useEffect(() => {
  ...
  const fetchAvailableTables = async (svId: number) => {
    try {
      ...
      if (response.ok) {
        ...
        const tables = schemaVersion.tables?.map((tbl: any) => tbl.table_name) || [];
        setAvailableTables(tables);

        const fieldOpts: {[key: string]: string[]} = {};
        schemaVersion.tables?.forEach((tbl: any) => {
          fieldOpts[tbl.table_name] = tbl.fields?.map((f: any) => f.field_name) || [];
        });
  ...
  // Generate available keys from fields that have constraints
  ...
  const handleTableNameChange = (value: string) => {
    setTableName(value);
    // Auto-generate file name short only in create mode
    if (!isEditMode) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // In edit mode, validate fileKeyName more strictly
    if (isEditMode) {
      if (!fileKeyName.trim()) {
    ...
    onSave(tableName, fields, fileKeyName, fileNameRenamed, fileNameShort);
  };
  ...
  const modalContent = (
    ...
    >
      <div
        className="table-modal rounded-lg p-6 w-full max-w-4xl mx-4 shadow-2xl max-h-[90vh] overflow-hidden flex flex-col"
          ...
          <div className="space-y-4">
              ...
              {isEditMode ? (
                <>
                  ...
                  <div className="text-xs mt-1" style={{ color: colors.textMuted }}>
                    {t.edittablemodal456}{'{filekeyname}'}{t.edittablemodal456_2}
                  </div>
                </>
              ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                />
                <div className="text-xs mt-1" style={{ color: colors.textMuted }}>
                  {isEditMode ? t.edittablemodal476 : t.createtablemodal374}{'{file_name_renamed}'}{isEditMode ? t.edittablemodal476_2 : t.createtablemodal374_2}
                </div>
              </div>
              ...
              <div>
                ...
                />
                <div className="text-xs mt-1" style={{ color: colors.textMuted }}>
                  {isEditMode ? t.edittablemodal495 : t.createtablemodal393}{'{file_name_short}'}{isEditMode ? t.edittablemodal495_2 : t.createtablemodal393_2}
                </div>
              </div>
            </div>
          </div>
